...
- irb, тип данных (.class) (String, Fixnum, Float, Integer), array. Hash)
- задача про сопротивление (закон Ома)
- преобразование типов (to_i переводим в to_s)
...
- задача: игра однорукий бандит (начало)
...
- устанавливаем datetimepicker
    ...
    ("Бортовой журнал" наблюдателя: 
    Проект обновился, нужно подключить новые файлы:
    1. Скачать jquery.datetimepicker.css.
    2. Скачать jquery.datetimepicker.full.min.js (находится в папке build).
    3. В представлении layout.erb прописать:
       <script src="jquery.datetimepicker.full.min.js"></script>
       <link rel="stylesheet" href="jquery.datetimepicker.css" />
    4. В представлении visit.erb такой же код как и в видео уроке)

- выборка из БД/ вывод инфы
...
- устанавливаем shoulda-matchers 
    ...
    ("Бортовой журнал" наблюдателя: 
    В момент просмотра ролика и выполнения урока (29.12.2015) у меня возникла проблема с shoulda-matchers. 
    Выскакивала ошибка: undefined method `validate_presence_of'. 
    Нашел следующее решение: в Gemfile прописываем # shoulda-matchers.
    Записываем в отдельную группу:

    group :test do
             gem "shoulda-matchers", '~> 2.5.0', require: false
           end

    в spec_helper.rb после require 'rspec/rails' прописываем: 

    require 'shoulda/matchers')

- устанавливаем factory_girl: простейшая настройка, создание фабрики, тестирование
